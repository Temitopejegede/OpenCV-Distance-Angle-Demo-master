...
class Vision:
    ...
    screenWidth = 1000  # Size of resized screen
    xmid = 0.0
    ymid = 0.0
    ...
    def getAngle(self):
        ...
        self.angle = ((self.xmid / (1830 - 94)) * 110)

        # 1830 and 94 are the farthest left and right x - positions the camera can see.

        if (self.angle > 110):
            self.angle = 110
        if (self.angle < 0):
            self.angle = 0
        return self.angle - 55
    ...
    def updateFrame(self, frame, low_H, low_S, low_V, high_H, high_S, high_V):
        ...
        for c in cnts:
            ...
            if area > threshold_area:
                ...
                self.fittedHeight = self.__calculateDistance(
                    box[0][0], box[0][1], box[1][0], box[1][1])
                self.fittedWidth = self.__calculateDistance(
                    box[2][0], box[2][1], box[1][0], box[1][1])
                ...
                # x = np.array([self.xmid])

                # y = np.array([self.ymid])
        ...
        self.xmid = x + (w/2)
        # print(self.xmid)
        # print(self.fittedWidth)
        self.ymid = y + (h/2)

        return mask, img
# based on location, track the angle and radius. To track
# which car is which, the episilon (change in distance) has to be the smallest
# compare the change in t and t1, and the one with the smallest distance is the one being tracked
# aLSO, averaging out the position of the cars in the last second is the where the car is right now
